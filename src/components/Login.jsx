...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Simulate a fetch API call to a mock endpoint
      const response = await fetch('https://jsonplaceholder.typicode.com/posts', {
        method: 'POST', // Simulating a login POST request
        headers: {
      ...
      // Simulate successful response (in a real app, you'd parse the actual response)
      const data = await response.json();
      console.log('Simulated API response:', data);

      // Check credentials (simulation - in reality, this would come from the server)
      if (email && password) {
    ...
    <div className="min-h-screen flex items-center justify-center  bg-[#1A1A2E] py-12 px-6 p-4">
      ...
      >
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          ...
          <a href="/SignUp" className="font-medium text-[#7AA1D2] hover:text-[#6B8BBF] transition-colors duration-200">
            Sign up
          </a>
        </p>
      </div>
